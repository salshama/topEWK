...
output_directory = "/ceph/salshamaily/topEWK_FCCee/samples/deviation"
...
def process_histograms():
    ...
    for root_file in root_files:
        ...
        if variable in ['Emiss_energy', 'muon_pass_energy', 'electron_pass_energy'] and variation == "SM":
            ...
            hist_sm_copy.SetName(f"{process}{variation}{variable}{selection}")
            hist_sm_copy.SetDirectory(0)
#             factor = hist_sm_copy.GetEntries() / hist_sm_copy.Integral() * (418000.0 / 6400000)
            print(f"{variation}, {variable}, {process}, entries: {hist_sm_copy.GetEntries()}, integral: {hist_sm_copy.Integral()} factor: {factor}")
            hist_sm_copy.Scale(factor)
